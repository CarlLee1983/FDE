# System-Analysis Methods That Can Inform FDE

**Purpose.** This memo identifies established methods that complement the project's FDE scenario registry, ontology assets, evidence-aware context resolution, decision support, controlled action, and governance. It is research input, not a claim that an FDE must certify against or fully implement any standard.
...
**FDE use.** Use BPMN for repeatable, prescriptive flows (handoffs, timers, approvals, integrations); use a case model when work is exception-driven and the next task depends on the evolving situation. In either, mark the exact process/case node at which the FDE scenario reads context, recommends a decision, or requests a controlled action.
...
**FDE use.** Express high-consequence or repeatable decisions as named decision services: required inputs, source/definition versions, decision logic or policy, output, owner, and escalation. Keep decision logic distinct from both the process that calls it and the agent interface that explains it.
...
| Where does the work and exception occur? | BPMN or CMMN | Process/case model with FDE intervention node | Users validate normal, exception, and escalation paths |
...
### Suggested order for the first FDE method pack
...
- BPMN/CMMN describe work coordination; DMN describes decision logic; UML state models describe allowed lifecycle transitions. An FDE should not force one notation to do all three jobs.
...
- NIST RMF is a security/privacy framework. Its control and authorization pattern is reusable, but an FDE should tailor it to the organisational and regulatory context.
...
- The **Source facts** sections summarize what those sources state. The **FDE use**, **Proposed FDE synthesis**, and **Deliberate boundaries** sections are the author’s design synthesis for this repository.
