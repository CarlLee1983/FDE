...
- `FDE-Scenario-to-Action-Method.zh-TW.md` gains the `協作釐清` section, which the translation had never carried; it now states the same fact-versus-decision split and the same requested-analysis rule as the English source. The translation's remaining divergences from the current English structure are untouched.
