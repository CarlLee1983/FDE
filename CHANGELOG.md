...
- The new-evidence reconciliation report now closes with a batch status that reports engineering verification, behaviour evaluation, and real-world application separately: `make verify` passing, round 3 independently adjudicated at 12 PASS and 1 FAIL, and no real enterprise trial evidence. The batch is not recorded as fully accepted, and original answers, original scores, re-adjudications, and dissents are preserved rather than overwritten.
- Two limitations are carried forward with their original evidence, effect, re-check point, and the observation that would trigger a further correction: evidence fields left untagged in one round-3 answer, and a completion level overstated as `只差核可` in another. Neither produced a new rule, and the earlier `run variance` classification of the first is withdrawn because its cause is not established.
- The evaluation contract now describes how to run a real-case trial as ordinary use — one scoped real problem, the five-part solution, new or contradicting information, a check on what was revised, and retention under the case's own access rules — with the observations that would justify a further change. Trial status is not started.
